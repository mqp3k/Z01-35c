...
def read_messages_to_user_from_date(receiver_id: int, sender_id: int, from_date: datetime, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    messages = crud.get_messages_to_user_from_date(db, receiver_id=receiver_id, sender_id=sender_id, date=from_date, skip=skip, limit=limit)
    return messages
