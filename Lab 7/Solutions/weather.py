...
def get_weather(city='wroclaw', index=0):
    ky = 'dfc2cb2adfed32b31e8e6c4217b27a0c'
    try:
        weather_json = requests.get(
            f'http://api.openweathermap.org/data/2.5/forecast?q={city}&units=metric&appid={ky}').json()
    except:
        return
...
def get_hourly_weather(city='wroclaw', start=0, count=8):
    ky = 'dfc2cb2adfed32b31e8e6c4217b27a0c'
    try:
        weather_json = requests.get(
            f'http://api.openweathermap.org/data/2.5/forecast?q={city}&units=metric&appid={ky}').json()
...
def get_img(img_symbol, size):
    img_url = f'http://openweathermap.org/img/wn/{img_symbol}@2x.png'
    response = requests.get(img_url)
    img_data = response.content
    img = ImageTk.PhotoImage(Image.open(BytesIO(img_data)).resize((size, size), Image.ANTIALIAS))
    return img, img_data
...
class WeatherApp:
    def __init__(self, city='Wroclaw'):
        self.window = tk.Tk()
        ...
        self.secondary_color = 'YELLOWGREEN'


        # TEMPERATURE FRAME
        self.label_frame_temp = tk.LabelFrame(self.master, text='Temperature')
        self.label_frame_temp.columnconfigure(0, weight=1)
        self.label_frame_temp.columnconfigure(1, weight=1)

        self.the_temp_label = tk.Label(self.label_frame_temp, anchor=tk.E, text='Now: ', width=12)
        self.the_temp_label.grid(column=0, row=0)
        self.the_temp = tk.Label(self.label_frame_temp, width=12, anchor=tk.W)
        ...
        self.min_temp = tk.Label(self.label_frame_temp, width=12, anchor=tk.W)
        ...
        self.max_temp = tk.Label(self.label_frame_temp, width=12, anchor=tk.W)
        self.max_temp.grid(column=1, row=2)

        # CONDITIONS FRAME
        self.label_frame_conditions = tk.LabelFrame(self.master, text='Conditions')
        ...
        self.air_pressure = tk.Label(self.label_frame_conditions, width=12, anchor=tk.W)
        ...
        self.visibility = tk.Label(self.label_frame_conditions, width=12, anchor=tk.W)
        ...
        self.humidity = tk.Scale(self.label_frame_conditions, orient=tk.VERTICAL, from_=100, to=0,
                                 troughcolor='LIGHTBLUE', sliderlength=5, length=50)
        self.humidity.grid(column=1, row=4, rowspan=2, sticky=tk.W)

        # WIND FRAME
        self.label_frame_wind = tk.LabelFrame(self.master, text='Wind')
        ...
        self.wind_direction_compass = tk.Label(self.label_frame_wind, width=12, anchor=tk.W)
        self.wind_direction_compass.grid(column=1, row=0)

        self.wind_speed_label = tk.Label(self.label_frame_wind, anchor=tk.E, text='Speed: ', width=12)
        self.wind_speed_label.grid(column=0, row=1)
        self.wind_speed = tk.Label(self.label_frame_wind, width=12, anchor=tk.W)
        self.wind_speed.grid(column=1, row=1)

        # LOCALIZATION FRAME
        self.label_frame_localization = tk.LabelFrame(self.master, text='Localization')
        ...
        self.country = tk.Label(self.label_frame_localization, width=12, anchor=tk.W)
        ...
        self.city = tk.Label(self.label_frame_localization, width=12, anchor=tk.W)
        ...
        self.time = tk.Label(self.label_frame_localization, width=12, anchor=tk.W)
        self.time.grid(column=1, row=2)

        # SUN FRAME
        self.label_frame_sun = tk.LabelFrame(self.master, text='Sun today')
        self.label_frame_sun.columnconfigure(0, weight=1)
        self.label_frame_sun.columnconfigure(1, weight=1)

        self.sun_rise_label = tk.Label(self.label_frame_sun, anchor=tk.E, text='Sun rise: ', width=12)
        self.sun_rise_label.grid(column=0, row=0)
        self.sun_rise = tk.Label(self.label_frame_sun, width=12)
        self.sun_rise.grid(column=1, row=0)

        self.sun_set_label = tk.Label(self.label_frame_sun, anchor=tk.E, text='Sun set: ', width=12)
        self.sun_set_label.grid(column=0, row=1)
        self.sun_set = tk.Label(self.label_frame_sun, width=12)
        self.sun_set.grid(column=1, row=1)

        # OVERALL FRAME
        self.label_frame_overall = tk.LabelFrame(self.master, text='Weather', bg=self.primary_color)
        self.label_frame_overall.columnconfigure(0, weight=1)
        self.label_frame_overall.columnconfigure(1, weight=1)

        self.date = tk.Label(self.label_frame_overall, width=24, font=tkFont.Font(weight='bold'), bg=self.primary_color)
        ...
        self.image = tk.Label(self.label_frame_overall, width=240, bg=self.primary_color)
        self.image.grid(column=0, row=2)

        self.temp_main = tk.Label(self.label_frame_overall, width=24, font=tkFont.Font(weight='bold'), bg=self.primary_color)
        self.temp_main.grid(column=0, row=3)

        # ENTRY FRAME
        self.frame_entry = tk.Frame(self.master, bg=self.secondary_color)
        ...
        self.info_text = tk.Label(self.frame_entry,
                                  text='Weather data comes from \nhttps://www.openweathermapmetaweather.org/',
                                  bg=self.secondary_color)
        self.info_text.grid(column=1, row=0, columnspan=1, rowspan=2)

        self.city_entry_label = tk.Label(self.frame_entry, anchor=tk.E, text='City: ', bg=self.secondary_color)
        ...
        self.today_button = tk.Button(self.frame_date_selector, text='Today',
                                      command=lambda: self.update_interface(self.curr_weather['city']), width=25)
        self.tomorrow_button = tk.Button(self.frame_date_selector, text='Tomorrow',
                                         command=lambda: self.update_interface(self.curr_weather['city'], 8), width=45)
    ...
    class HourFrame(tk.LabelFrame):
        def __init__(self, parent, primary_color='SKYBLUE'):
            ...
            self.image_label = tk.Label(self, width=11, text='image', bg=primary_color)
        ...
        def update_hour(self, weather_stats, min_temp, max_temp):
            ...
            self.img, self.img_data = get_img(weather_stats['icon'], 20)
    ...
    def update_weather(self, city, index=0):
        ...
        try:
            ...
            tk.messagebox.showwarning('Incorrect city', 'Please! Provide correct city.')
